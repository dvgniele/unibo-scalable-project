...
set PROJECT_ID=scala-unibo
set CLUSTER_NAME=brave-cluster-1k
set BUCKET_NAME=brave-bucket-d
...
@REM call gcloud storage buckets create gs://%BUCKET_NAME% --project=%PROJECT_ID% --default-storage-class=%STORAGE% --location=%LOCATION% --uniform-bucket-level-access
...
@REM call gcloud iam service-accounts create %SA_NAME% --description=%SA_DESCRIPTION% --display-name=%SA_DISPLAY_NAME%
...
@REM call gcloud projects add-iam-policy-binding %PROJECT_ID% --member=%MEMBER% --role=%ROLE%
...
@REM call gcloud iam service-accounts keys create %KEY_FILE% --iam-account=%SA_NAME%@%PROJECT_ID%.iam.gserviceaccount.com
...
@REM call gcloud dataproc clusters create %CLUSTER_NAME% --enable-component-gateway --bucket %BUCKET_NAME% --region %REGION% --zone %ZONE% --single-node --master-machine-type n1-custom-%CORES%-%MEMORY% --master-boot-disk-size 500 --image-version 2.1-debian11 --project %PROJECT_ID%

echo Uploading the key

set CLUSTER_PATH=/home/ktm17

@REM call gcloud compute scp %KEY_FILE% %CLUSTER_NAME%-m:%CLUSTER_PATH% --zone=%ZONE%
...
@REM call gsutil cp %KEY_FILE% gs://%BUCKET_NAME%/
@REM call gsutil cp %JAR_PATH% gs://%BUCKET_NAME%/
@REM call gsutil -m cp -R %DATASET_PATH% gs://%BUCKET_NAME%/
